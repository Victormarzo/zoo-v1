...
export default function AnimalPage({animais, onSubmit, loading, error, onError}){
    ...
    const renderAnimalList = () => {
        if (loading) return <StatusMessage message="Carregando dados dos animais..." />;
        if (error) return <StatusMessage message={error} type="error" />;
        if (animais.length === 0) return <StatusMessage message="Nenhum animal cadastrado. Comece adicionando um novo!" type="info" />;
    ...
    const renderAnimalForm = () => {
        return (
            <FormContainer onSubmit={handleLocalSubmit}>
                <FormHeader>{'Cadastrar Novo Animal'}</FormHeader>
                {formErrors && (
                    <FormItemFull>
                        <StatusMessage
                ...
                <FormItem >
                    <FormLabel htmlFor="species" >Espécie *</FormLabel>
                    <FormInput type="text" id="species" name="species" value={formData.species} onChange={handleFormChange} required/>
                </FormItem>

                <FormItem >
                    <FormLabel htmlFor="birth_date" >Data de Nascimento</FormLabel>
                    <FormInput type="date" id="birth_date" name="birth_date" value={formData.birth_date} onChange={handleFormChange}/>
                </FormItem>

                <FormItem >
                    <FormLabel htmlFor="habitat" >Habitat</FormLabel>
                    <FormInput type="text" id="habitat" name="habitat" value={formData.habitat} onChange={handleFormChange}/>
                </FormItem>
                
                <FormItem >
                    <FormLabel htmlFor="country_origin" >País de Origem</FormLabel>
                    <FormInput type="text" id="country_origin" name="country_origin" value={formData.country_origin} onChange={handleFormChange}/>
                </FormItem>
    ...
    return(
        <>
            <ShowFormButton buttonsucess={'sucess'} onClick={() => setShowForm(!showForm)}> 
                <PlusCircle size={18} style={{ marginRight: 5 }} /> {showForm ? 'Esconder Formulário' : 'Adicionar Novo Animal'}
                <ChevronDown size={18} style={{ marginLeft: 10, transition: 'transform 0.3s', transform: showForm ? 'rotate(180deg)' : 'rotate(0deg)' }}/>
            </ShowFormButton>
            
            {showForm && renderAnimalForm()}
            {renderAnimalList()}
        </>
    )
}
